...
import CircleSnake3.lib.utils.circle.kidpath_circle as kidpath_circle
# from CircleSnake3.external.cityscapesscripts.evaluation import \
#     evalInstanceLevelSemanticLabeling
from CircleSnake3.lib.config import cfg
from CircleSnake3.lib.datasets.dataset_catalog import DatasetCatalog
from CircleSnake3.lib.utils import data_utils
from CircleSnake3.lib.utils.circle.circle_eval import CIRCLEeval
from CircleSnake3.lib.utils.snake import (snake_cityscapes_utils, snake_config,
...
class Evaluator:
    def __init__(self, result_dir):
        ...
        self.contiguous_category_id_to_json_id = {category_id: category_id for category_id in self.json_category_id_to_contiguous_id.values()}
    ...
    def evaluate(self, output, batch):
        ...
        if cfg.debug_test:
            ...
            if cfg.save_images:
                path = os.path.join(
                    "/home/VANDERBILT/liuy99/Documents/EOS/CircleSnake3/data/debug/predicted"
                )
                ...
                path = os.path.join(
                    "/home/VANDERBILT/liuy99/Documents/EOS/CircleSnake3/data/debug/truth"
                )
                if not os.path.exists(path):
                    os.makedirs(path)
    ...
    def summarize(self):
        ...
        # per_category_aps = coco_eval.eval['precision']
        # for category_id in range(len(self.coco.cats)):
        #     if category_id in self.coco.cats:
        #         category_name = self.coco.cats[category_id]['name']
        #         aps = per_category_aps[category_id, :, :]
        #         print(f"AP for {category_name}:")
        #         for iou_idx, ap in enumerate(aps[0, :, -1]):
        #             print(f"IoU threshold {coco_eval.params.iouThrs[iou_idx]}: {ap}")
        #     else:
        #         print(f"No annotations found for category ID {category_id}")
