...
def densepose_chart_predictor_output_to_result(
    ...
    Args:
        predictor_output (DensePoseChartPredictorOutput): DensePose predictor
            output1 to be converted to results, must contain only 1 output1
        boxes (Boxes): bounding box that corresponds to the predictor output1,
            must contain only 1 bounding box
    Return:
       DensePose chart-based result (DensePoseChartResult)
    """
    assert len(predictor_output) == 1 and len(boxes) == 1, (
        f"Predictor output1 to result conversion can operate only single outputs"
...
def densepose_chart_predictor_output_to_result_with_confidences(
    ...
    Args:
        predictor_output (DensePoseChartPredictorOutput): DensePose predictor
            output1 with confidences to be converted to results, must contain only 1 output1
        boxes (Boxes): bounding box that corresponds to the predictor output1,
            must contain only 1 bounding box
    Return:
