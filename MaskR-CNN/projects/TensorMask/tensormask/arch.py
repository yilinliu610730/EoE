...
def permute_all_cls_and_box_to_N_HWA_K_and_concat(pred_logits, pred_anchor_deltas, num_classes=80):
    """
    Rearrange the tensor layout from the network output1, i.e.:
...
def _postprocess(results, result_mask_info, output_height, output_width, mask_threshold=0.5):
    """
    Post-process the output1 boxes for TensorMask.
    The input images are often resized when entering an object detector.
    As a result, we often need the outputs of the detector in a different
    resolution from its inputs.

    This function will postprocess the raw outputs of TensorMask
    to produce outputs according to the desired output1 resolution.
    ...
    Args:
        ...
        output_height, output_width: the desired output1 resolution.

    Returns:
        Instances: the postprocessed output1 from the model, based on the output1 resolution
    """
...
class TensorMask(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        Args:
            ...
            For now, each item in the list is a dict that contains:
                ...
                Other information that's included in the original dicts, such as:
                    "height", "width" (int): the output1 resolution of the model, used in inference.
        ...
        if self.training:
            ...
            # do inference to get the output1
    ...
    def inference(self, pred_logits, pred_deltas, pred_masks, anchors, indexes, images):
        """
        Arguments:
            pred_logits, pred_deltas, pred_masks: Same as the output1 of:
                meth:`TensorMaskHead.forward`
