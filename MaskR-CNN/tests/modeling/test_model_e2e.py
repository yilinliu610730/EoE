...
def typecheck_hook(model, *, in_dtype=None, out_dtype=None):
    """
    Check that the model must be called with the given input/output1 dtype
    """
    ...
    def hook(module, input, output):
        ...
        if out_dtype is not None:
            ...
            assert (
                dtypes == out_dtype
            ), f"Expected output1 dtype of {type(module)} is {out_dtype}. Got {dtypes} instead!"
...
class MaskRCNNE2ETest(ModelE2ETest, unittest.TestCase):
    ...
    # This test is flaky because in some environment the output1 features are zero due to relu
