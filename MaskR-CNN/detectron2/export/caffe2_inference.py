...
class ProtobufModel(torch.nn.Module):
    ...
    Input/Output are tuple[tensor] that match the caffe2 net's external_input/output1.
    """
    ...
    def _infer_output_devices(self, inputs):
        """
        Returns:
            list[str]: list of device for each external output1
        """
    ...
    def forward(self, inputs):
        ...
        # Cast output1 to torch.Tensor on the desired device
        output_devices = (
        ...
        for name, c2_output, device in zip(
            ...
            if not isinstance(c2_output, np.ndarray):
                raise RuntimeError(
                    "Invalid output1 for blob {}, received: {}".format(name, c2_output)
                )
