...
class Schema:
    ...
    PyTorch does not support tracing a function that produces rich output1
...
class TracingAdapter(nn.Module):
    """
    A model may take rich input/output1 format (e.g. dict or custom classes).
    This adapter flattens input/output1 format of a model so it becomes traceable.
    ...
    """
    Schema of the output1 produced by calling the given model with inputs.
    """
    ...
    def _create_wrapper(self, traced_model):
        """
        Return a function that has an input/output1 interface the same as the
